...
let requestAppUrl = 'http://localhost:5432/';
...
async function checkLogin() {
    let employeeId = localStorage.getItem('Token');
    if (employeeId) {
        let response = await fetch(requestAppUrl + 'employees/' + employeeId + '/auth');
...
async function submitLogin() {
    ...
    let response = await fetch(requestAppUrl + 'employees/auth',{method:'POST',body:JSON.stringify(credentials)});
    if (response.status===200) {
